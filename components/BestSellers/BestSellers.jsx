...
const BestSellers = () => {
  return (
    <section className="max-w-7xl mx-auto mb-12 px-8 md:px-8 xl:rounded-xl h-112.5 bg-gray-200">
      {/* Header */}
      ...
      >
        {bestSellers.map((item) => (
          <SwiperSlide key={item.id}>
            <a
              href={item.link}
              className="flex flex-col justify-between rounded-xl bg-white shadow-md hover:shadow-lg transition-shadow duration-300 relative h-90"
            >
              {/* Status Dots */}
              ...
              <img
                src={item.image}
                alt={item.title}
                className="mx-auto rounded-t-xl w-50 h-50 object-contain"
              />
